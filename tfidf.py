...
def main():
    ...
    #SVM
    clf = SVC(decision_function_shape='ovv',kernel='rbf')
    clf.fit(train_x, train_y)
    print("*** SVM kernel : RBF ***")
    print("train acc : {}".format(clf.score(train_x,train_y)))
    print("test acc : {}".format(clf.score(test_x,test_y)))
    print("confusion matrix :\ntrue\predict")
    labels = list(Counter(test_y).keys())
    confu_mtx = confusion_matrix(test_y,clf.predict(test_x),labels=labels)
    for i,j in zip(labels,confu_mtx):
        print("{:8}".format(i),format(j))
